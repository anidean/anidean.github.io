...
Make sure you have an account on Vultr or some other cloud provider with $5 instances.

### Part 2


Make sure you have an SSH key on your local machine.

Go to your Vultr Account and create a startup script.  Paste the below code in it.  Call it 'setup' then save and go back the products page.
...
Back at the products page deploy a server anywhere you want.
...
Copy the ip address and SSH into the server when it's running, you'll only have to wait a few seconds.
```sh
  ssh ssh_username@123.456.789.012
```

If the previous step worked Open up a new terminal and scp the 'install_podman.sh' script to the server with the code below.  If not, then figure out why it didn't work.
```sh
  scp install_podman.sh ssh_username@123.456.789.012:~/
```

In your previous terminal become sudo with:
```sh
  sudo su
```
...
untar the file, cd into the directory and modify the Makefile to just copy the completed files to the server.
```sh
